...
let _logIdCounter = 0;
export const nextLogId = () => `log-${++_logIdCounter}`;
...
export function dedupeLogs(logs: GameState['logs']): GameState['logs'] {
...
async function resolveHunterShoots(
  ...
  for (const deadId of hunterDeadIds) {
    ...
    const shotId = await aiHunterChooseTarget(state, deadId, isNightDeath);
...
function autoSkipPhases() {
  ...
  if (state.phase.startsWith('night-') && state.phase !== 'night-result') {
...
function buildDiscussionOrder(state: GameState): string[] {
  ...
  // 安全检查：确保人类玩家（如果存活）一定在发言顺序中
  const humanPlayer = state.players.find(p => !p.isAI);
  if (humanPlayer && humanPlayer.isAlive && !order.includes(humanPlayer.id)) {
    console.warn('[buildDiscussionOrder] 人类玩家未在发言顺序中，已自动添加', humanPlayer.name);
    // 将人类玩家插入到合适位置（当前顺序的末尾）
    order.push(humanPlayer.id);
  }
...
function startNextSpeaker() {
  ...
  if (speaker.isAI) {
    ...
    // 人类玩家：等待用户输入
    console.log(`[startNextSpeaker] 轮到人类玩家 ${speaker.name} 发言 (index=${currentIdx}/${order.length}), 等待用户输入...`);
  }
...
你的遗言内容`;
  ...
  if (!content) {
    // LLM 失败或返回内容不符合要求，使用兜底遗言
    const fallbacks = lastWordsFallbacks[exiledPlayer.role] || lastWordsFallbacks.villager;
