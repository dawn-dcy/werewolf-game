...
export function getAIName(player: Player): string {
...
function buildGameRules(state: GameState): string {
  ...
  const specialRules: string[] = [
    '- ⚠️ **【核心规则】狼人每夜必须击杀一名玩家，绝对不能空刀！系统强制狼人选目标，不存在"狼人空刀"或"狼人故意不杀人"的可能性，本游戏中绝对不可能发生空刀！**',
  ];
  ...
  specialRules.push('- ⚠️ 遗言声明：被投票放逐或被猎人开枪带走的玩家可以发表遗言，但遗言内容及其声称的身份均不一定真实，可能包含误导或虚假信息，请各位玩家根据游戏局势自行判断其可信度');
  specialRules.push('- 🔒 **刀口保密规则**：每晚狼人袭击的目标（刀口）只有两类玩家知道：①狼人（刀口由他们自己选定）；②解药尚未使用的女巫（系统会明确告知刀口是谁）。被狼人袭击的玩家本人——无论他是因此死亡，还是被守卫守护或女巫解药救下而活了下来——都完全不知道自己曾是当晚的刀口！任何玩家发言都不得声称"我昨晚被刀了/我是刀口"，也不得断言某位幸存玩家昨晚是否被刀。');
...
- 你的策略：
  1. 夜晚时与队友协商选择击杀目标，可以自刀骗解药；⚠️ 每夜必须杀人，不能空刀（系统强制）
...
- 你的策略：
  ...
  8. ⚠️ **跳预言家后的发言纪律**：一旦你在白天讨论中公开跳了预言家身份，从那一轮起，你**每一轮的白天讨论发言**都必须说清楚三件事：
     ① **昨晚的验人信息**：验了谁、结果是什么（查杀=狼人，金水=好人；如果是第一轮刚跳身份，直接报你手头的查验结果）；
     ② **验人逻辑**：为什么昨晚验这个人（他的发言哪里可疑、哪条线索指向他、上一轮交代的验人安排是谁）；
     ③ **晚上的验人安排**：今晚你打算验谁、为什么。
     每一晚的查验结果都要能对上你上一轮公开交代过的"验人安排"，前后自洽、信息完整，这样好人才会相信你的预言家身份；切忌只报一句"验了X是好人/狼人"，却不讲验人逻辑和后续验人安排，更不要出现验人对象前后矛盾的情况。
- ⚠️ 唯一身份规则：所有神职都只有1个！如果有人自称预言家或声称和你角色相同，那他100%是假的！可能是狼人悍跳预言家（这是狼人最常用的战术之一），也可能是村民想帮你挡刀，但大概率是狼人悍跳。遇到有人和你对跳预言家，你要坚决反击、揭穿他！
- 注意事项：你是好人的眼睛，你的信息至关重要！不要浪费查验机会，但也要在适当时机把信息传给好人阵营！记住：既然跳了预言家身份，每一轮发言都要报全"验人信息＋验人逻辑＋晚上的验人安排"，含糊其辞或逻辑断裂都会让你被好人当成悍跳狼！`;
...
- ⚠️ 唯一身份规则：所有神职都只有1个！如果游戏进行到中后期，有人跳女巫身份，那他100%是假的——因为真正的女巫是你！更大的可能是狼人悍跳女巫来混淆视听。此时你可以考虑跳身份揭穿他，但也要权衡暴露自己被刀的风险。
...
- ⚠️ 唯一身份规则：所有神职都只有1个！如果有人在讨论中自称守卫，那他100%是假的——因为真正的守卫是你！很可能是狼人悍跳守卫。如果时机合适，可以考虑揭穿他，但要小心暴露自己。
...
- ⚠️ 唯一身份规则：所有神职都只有1个！如果有人在讨论中自称猎人，那他100%是假的——因为真正的猎人是你！更大的可能是狼人悍跳猎人来混淆好人阵营的判断。如果你愿意暴露身份，可以直接揭穿他。
...
- 🔒 **刀口保密**：每晚狼人袭击的目标（刀口）只有狼人自己和解药尚未使用的女巫知道；被袭击的玩家本人即使因守卫/解药保护而活下来，也不知道自己曾被刀。发言中不得声称自己昨晚被刀，也不得断言他人是否被刀。
 ...
 * 守卫：每轮守护了谁
 */
export function buildPlayerActionHistory(state: GameState, player: Player): string {
...
${killedPlayer.id === player.id
  ...
  : `你需要决定是否使用解药救活「${getAIName(killedPlayer)}」。`}`
  : player.hasAntidote
 ...
 * @param isNightDeath true=夜晚被狼人杀害触发，false=白天被投票放逐触发
 */
export async function aiHunterChooseTarget(state: GameState, hunterId: string, isNightDeath: boolean = false): Promise<string | null> {
...
/**
 * 为出局的 AI 玩家构建遗言的多消息上下文
 * 结构：system(规则+遗言时刻) → 稳定历史消息序列 → user(存活/角色私密+历史操作) → user(当前状态+出局原因+遗言要求)
 * @param deathReason 出局原因：exiled=白天被投票放逐；shot=被猎人开枪带走
 */
export function buildLastWordsMessages(
  state: GameState,
  exiledPlayer: Player,
  deathReason: 'exiled' | 'shot' = 'exiled'
): ChatMessage[] {
  const roleName = ROLE_NAMES[exiledPlayer.role];

  // 反查"被猎人带走"场景下的开枪猎人（日志精确匹配「带走了 X」，避免把被放逐的猎人本身误判为被枪杀）
  let shooterName = '';
  if (deathReason === 'shot') {
    const shotLog = state.logs.find(
      l => l.round === state.round && l.message.includes(`带走了 ${exiledPlayer.name}`)
    );
    const m = shotLog?.message.match(/^(.+?)（猎人）在临死前开枪带走了 (.+?)！/);
    if (m) shooterName = m[1].replace(/\(你\)$/, '');
  }
  ...
  return [
    {
      ...
      content: buildSystemPrompt(state)
        + '\n\n你刚刚在白天出局了，现在是你发表遗言的时刻。请以第一人称，基于你的真实处境发表一段遗言。',
    },
    ...buildGameHistoryMessages(state),
...
${
  deathReason === 'shot'
    ? `### ⚠️ 你被猎人开枪带走了！
- 带走你的猎人：${shooterName || '一名猎人'}(开枪时其猎人身份已向全场公开)
- 你并不是被投票放逐的`
    : `### ⚠️ 你被投票放逐出局了！
- 你获得了 ${voteCount} 票
...
---
你刚刚出局，这是你最后一次发言机会。请发表一段有质量的遗言。

遗言要求：
- ${
  deathReason === 'shot'
    ? '你是被猎人开枪带走的，遗言中不要说自己"被投票放逐"；可以分析自己为何会被猎人盯上、怀疑谁在背后推动局势'
    : '可以基于讨论中的信息和投票结果来分析场上局势，说明为什么会被大家怀疑，或为好人阵营留下有价值的线索'
}
